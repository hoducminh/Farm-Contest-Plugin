...
public class FarmContestCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "shop"     -> handleShop(sender); // Lệnh mở cửa hàng thu mua nông sản đột biến
            case "mutation" -> handleMutation(sender, args); // 👈 Lệnh xử lý Nông sản Đột biến
    ...
    /** /fc profile [player] — xem hồ sơ nông dân từ SQLite */
    private void handleProfile(CommandSender sender, String[] args) {
        ...
        FarmProfile profile = plugin.getDatabaseManager().getProfile(targetUuid);

        send(sender, configManager.getMessage("profile-header"));
        send(sender, configManager.getMessage("profile-name").replace("{player}", targetName));
    ...
    /** /fc shop — Mở GUI thu mua nông sản đột biến */
